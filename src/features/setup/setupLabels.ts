...
export const getMatchModeLabel = (mode: MatchMode) => (
  mode === 'LEGS' ? 'Manches' : 'Sets'
);
...
export const getSetupGameName = (gameType: GameType) => {
...
export const getSetupRulesContent = (
  gameType: GameType,
  context: { checkIn: InOutRule; checkOut: InOutRule; cricketRounds: number }
): SetupRulesContent => {
  if (gameType === 'X01' || gameType === 'X01_501_BO5') {
    return {
      ...
      items: [
        ...
        `Check-in actuel : ${context.checkIn}. Le score ${context.checkIn === 'Open' ? 'commence immediatement' : 'ne commence qu apres une ouverture valide'}.`,
        `Check-out actuel : ${context.checkOut}. Il faut donc terminer selon cette regle pour valider la victoire.`,
  ...
  if (gameType === 'CRICKET') {
    return {
      ...
      items: [
        ...
        `La partie est limitee a ${context.cricketRounds} tours par joueur ou equipe, avec 3 fleches par tour.`,
      ],
    };
  }
