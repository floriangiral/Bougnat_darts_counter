...
export const TriathlonGameView: React.FC<TriathlonGameViewProps> = ({ players, onExit, onFinish }) => {
    ...
    const renderFloatingScoreboard = () => (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 bg-black/90 backdrop-blur-md border border-gray-700 rounded-full px-6 py-3 flex items-center gap-6 shadow-[0_0_30px_rgba(0,0,0,0.8)]">
            <div className="text-yellow-500 font-black italic text-sm uppercase tracking-widest mr-2 flex items-center gap-2">
                <span>🏆</span> Triathlon
            </div>
            {players.map(p => (
                <div key={p.id} className="text-white font-bold text-sm flex items-center gap-2">
    ...
    if (phase === 'TRANSITION_CRICKET') {
        return (
            <div className="h-[100dvh] bg-black text-white flex flex-col items-center justify-center p-6">
                <h1 className="text-5xl font-black italic text-transparent bg-clip-text bg-gradient-to-r from-blue-500 to-cyan-500 mb-8 text-center uppercase">
                    Étape 1 Terminée !
                </h1>
                <div className="bg-gray-900 border border-gray-800 rounded-2xl p-8 w-full max-w-md mb-12 shadow-2xl">
                    <h2 className="text-xl text-gray-400 font-bold uppercase tracking-widest mb-6 text-center">Score Global</h2>
                    <div className="space-y-4">
                        {players.sort((a,b) => globalScores[b.id] - globalScores[a.id]).map(p => (
                            <div key={p.id} className="flex justify-between items-center bg-gray-800 p-4 rounded-xl">
                                <span className="font-bold text-lg">{p.name}</span>
                                <span className="text-2xl font-black text-orange-500">{globalScores[p.id]} pts</span>
                            </div>
                        ))}
                    </div>
                </div>
                <Button onClick={() => setPhase('CRICKET')} size="lg" className="w-full max-w-md h-20 text-2xl uppercase shadow-lg shadow-green-900/40 bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-500 hover:to-emerald-500 border-none">
                    Étape 2 : Cricket ➔
                </Button>
            </div>
        );
    ...
    if (phase === 'TRANSITION_CAPITAL') {
        return (
            <div className="h-[100dvh] bg-black text-white flex flex-col items-center justify-center p-6">
                <h1 className="text-5xl font-black italic text-transparent bg-clip-text bg-gradient-to-r from-green-500 to-emerald-500 mb-8 text-center uppercase">
                    Étape 2 Terminée !
                </h1>
                <div className="bg-gray-900 border border-gray-800 rounded-2xl p-8 w-full max-w-md mb-8 shadow-2xl">
                    <h2 className="text-xl text-gray-400 font-bold uppercase tracking-widest mb-6 text-center">Score Global</h2>
                    <div className="space-y-4">
                        {players.sort((a,b) => globalScores[b.id] - globalScores[a.id]).map(p => (
                            <div key={p.id} className="flex justify-between items-center bg-gray-800 p-4 rounded-xl">
                ...
                <p className="text-center text-gray-400 mb-8 max-w-md italic">
                    Le Capital vaut <strong className="text-orange-500">3 points</strong>. Tout est encore possible !
                </p>
                <Button onClick={() => setPhase('CAPITAL')} size="lg" className="w-full max-w-md h-20 text-2xl uppercase shadow-lg shadow-red-900/40 bg-gradient-to-r from-red-600 to-orange-600 hover:from-red-500 hover:to-orange-500 border-none">
                    Étape 3 : Capital ➔
                </Button>
            </div>
        );
    ...
    if (phase === 'TIE_BREAKER') {
        return (
            <div className="h-[100dvh] bg-black text-white flex flex-col items-center justify-center p-6">
                <h1 className="text-5xl font-black italic text-transparent bg-clip-text bg-gradient-to-r from-red-500 to-yellow-500 mb-4 text-center uppercase animate-pulse">
                    ÉGALITÉ PARFAITE !
                </h1>
                ...
                <div className="w-full max-w-md space-y-6 mb-12">
                    {tiedPlayers.map(p => (
                        <div key={p.id} className="bg-gray-900 p-6 rounded-2xl border border-gray-800 flex flex-col gap-4">
                            <label className="font-bold text-xl text-center">{p.name}</label>
                            <input
                                type="number"
                                placeholder="Score (ex: 50)"
                                value={tieScores[p.id] || ''}
                                onChange={e => setTieScores({...tieScores, [p.id]: e.target.value})}
                                className="bg-black border border-gray-700 rounded-xl p-4 text-center text-3xl font-black text-orange-500 focus:outline-none focus:border-orange-500"
                            />
                        </div>
                    ))}
                </div>
                <Button onClick={handleTieBreakerSubmit} size="lg" className="w-full max-w-md h-20 text-2xl uppercase shadow-lg shadow-orange-900/40">
